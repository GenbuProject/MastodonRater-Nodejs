...
module.exports = class APIHandler {
		...
		return (function looper (max_id) {
			...
			return Mstdn.get(`accounts/${userId}/following`, props).then(info => {
				...
				if (nextLink) {
					...
					return new Promise(resolve => resolve(following));
				}
			});
		})();
	}
	...
	getFavourites (userId = 0, startDate = new Date(), endDate = new Date()) {
		const { Mstdn } = this;
		const start = Math.max(startDate, endDate);
		const end = Math.min(startDate, endDate);
		...
		return (function looper (max_id) {
			...
			return Mstdn.get("favourites", props).then(info => {
				for (let status of info.data) {
					const createdAt = new Date(status.created_at);

					if (start >= createdAt.getTime() && createdAt.getTime() >= end) {
						favourites.push(status);
					} else {
						return new Promise(resolve => resolve(favourites));
					}
				}
